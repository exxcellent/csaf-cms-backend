...
- Remove the CouchDB-specific query layer (Mango selectors), which does not
  support server-side paging, joins, or transactions.
...
| 3     | Entities       | One JPA entity per table                                                                                                                 |
| 4     | Repositories   | One Spring Data repository per entity, wrapped by a single bridge service                                                                |
| 5     | Service layer  | `AdvisoryService` rewritten against the Phase 4 bridge service; Mango selectors replaced by in-application filter evaluation (Section 4) |
| 6     | Configuration  | Flyway owns the DB schema; Hibernate runs in validation-only mode (Section 4)                                                            |
...
out of the main application: a separate import tool reads every document
out of the old CouchDB database over its plain HTTP API, groups it by type,
and writes the equivalent rows into PostgreSQL, honoring the same
foreign-key relationships as the schema in Section 3.
...
| Search predicates pushed down into SQL (Section 4) | Open, non-urgent improvement - the current approach is correct, just not the most efficient when handling large amount of data. |
