...
public class AdvisoryWrapper {
    ...
    String calculateReferenceUrl(String baseUrl, String trackingId) {
        int year = calculatePublishYear();
        String fileName = calculateFileName(trackingId, ".json");
    ...
    /**
     * Generate the HTML url for the given baseUrl and tracking id.
     * Format: BaseURL/TLPLabel/YearOfPublication/trackingId.html
     * @param baseUrl configured base url
     * @param trackingId the trackingid
     * @return the calculated HTML url
     */
    String calculateHtmlReferenceUrl(String baseUrl, String trackingId) {
        int year = calculatePublishYear();
        String fileName = calculateFileName(trackingId, ".html");
        String tlpLabel = getDocumentDistributionTlp() != null ? getDocumentDistributionTlp() : "WHITE";
        return baseUrl + "/" + tlpLabel.toLowerCase() + "/" + year + "/" + fileName;
    }
     ...
     * @param fileEnding the file ending, e.g. .json or .html
     * @return the filename
     */
    static String calculateFileName(String trackingId, String fileEnding) {
        String documentName = trackingId.toLowerCase(Locale.ENGLISH).replaceAll("[^+\\-a-z0-9]+", "_");
        return documentName + fileEnding;
    }
}
